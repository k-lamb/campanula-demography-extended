...
PMmod=open('./outputs/%s_NOMIG_output.txt' % Pair_name,'w')
PMmod.write(
...
def split_nomig_moments(params, ns, pop_ids=None):
...
func_moments = split_nomig_moments

#parameters are: (nu1, nu2, Ts)
upper_bound = [10, 10, 5]
...
for i in range(int(iterations)):
    ...
    PMmod=open('./outputs/%s_NOMIG_output.txt' % Pair_name,'a')
    PMmod.write(
    	str(Pair_name)+'\t'+ #pair name
        str(popt[0])+'\t'+ #nu1
        str(popt[1])+'\t'+ #nu2
