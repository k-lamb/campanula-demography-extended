...
PMmod=open('./outputs/%s_SC_output.txt' % Pair_name,'w')
PMmod.write(
...
def split_sc_moments(params, ns, pop_ids=None):
    ...
    fs.integrate([nu1, nu2], Tsc, m=np.array([[0, m12], [m21, 0]]))
    fs.pop_ids = pop_ids
    return fs

func_moments = split_sc_moments

#parameters are: (nu1, nu2, Ts, Tsc, m12, m21)
upper_bound = [10, 10, 5, 5, 20, 20]
lower_bound = [1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3]

for i in range(int(iterations)):
    print("starting optimization "+str(i))
    params = len(["nu1", "nu2", "Ts", "Tsc", "m12", "m21"])
    ...
    PMmod=open('./outputs/%s_SC_output.txt' % Pair_name,'a')
    PMmod.write(
    	str(Pair_name)+'\t'+ #pair name
        str(popt[0])+'\t'+ #nu1
        str(popt[1])+'\t'+ #nu2
